...
- Установка без прав администратора, удаление через «Параметры → Приложения»

## Как пользоваться

1. Скачайте установщик из [Releases](https://github.com/denis-fm/WorkSession-bin/releases/latest).
2. Запустите `WorkSession-Setup-1.0.0.exe`.
3. При желании отметьте ярлык на рабочем столе и вопрос при входе в Windows.
...
`WorkSession-Setup-1.0.0.exe`

```
2695A99D1EEFC0D8492CDE969747E7D7722A05FD1AB323343B6EF9D105ABEDB6
```

Проверка в PowerShell:

```powershell
Get-FileHash .\WorkSession-Setup-1.0.0.exe -Algorithm SHA256
```
...
- Per-user install, no administrator rights; uninstall from Settings → Apps

## How to use

1. Download the installer from [Releases](https://github.com/denis-fm/WorkSession-bin/releases/latest).
2. Run `WorkSession-Setup-1.0.0.exe`.
3. Optionally create a desktop shortcut and enable the logon prompt.
